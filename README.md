...
Ensure that the keys in the `sourceBinding` match those in the `sourceSettings` above! 
Otherwise, if there is no match, the `sourceBinding` will use first source specified in `sourceSettings`.

If the `patientGender` was connected to file system in the job definition, the `sourceBinding` parameters would be as in the following:
```json
{
  "mappings" : [ {
    "mappingRef" : "http://patient-mapping-with-two-sources",
