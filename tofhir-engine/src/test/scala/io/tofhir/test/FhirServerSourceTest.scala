...
class FhirServerSourceTest extends AsyncFlatSpec with BeforeAndAfterAll with ToFhirTestSpec with OnFhirTestContainer {
  ...
  val fhirServerSourceSettings: Map[String, FhirServerSourceSettings] =
    Map(
      "source" ->
        FhirServerSourceSettings(name = "test-fhir-server-source", sourceUri = "https://test-data", serverUrl = onFhirClient.getBaseUrl())
    )
  ...
  // Observation resource to be created on onFhir server as the source data
  ...
  /**
   * Create "example-observation" resource on the onFHIR server before starting the tests.
   * */
  ...
  /**
   * After the tests complete, delete the "example-observation" resource on the onFHIR server.
   * */
  ...
  /**
   * Tests the mapping job which reads data from the onFHIR and executes the mapping on this data.
  ...
  /**
   * Executes the mapping job, which reads data from OnFHIR and writes the generated resources back to OnFHIR.
   * After the mapping job is completed, there should be three Observation resources on OnFHIR: one Observation resource
   * as the source data and two Observation resources as the generated FHIR resources.
   */
  it should "map test data and write it to FHIR repo successfully" in {
    fhirMappingJobManager
      ...
      .flatMap(_ => {
        onFhirClient.search("Observation").where("subject", "Patient/example-patient-fhir")
          .executeAndReturnBundle() flatMap { obsBundle =>
          obsBundle.searchResults.size shouldBe 3
