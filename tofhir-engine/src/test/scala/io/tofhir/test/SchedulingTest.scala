...
class SchedulingTest extends AnyFlatSpec with BeforeAndAfterAll with ToFhirTestSpec {
  ...
  it should "schedule a FhirMappingJob with cron and sink settings restored from a file" in {
    ...
    val fhirMappingJobManager = new FhirMappingJobManager(mappingRepository, new MappingContextLoader(mappingRepository), schemaRepository, Map(FhirPathUtilFunctionsFactory.defaultPrefix -> FhirPathUtilFunctionsFactory), sparkSession, lMappingJob.mappingErrorHandling, runningJobRegistry, Some(mappingJobScheduler))
